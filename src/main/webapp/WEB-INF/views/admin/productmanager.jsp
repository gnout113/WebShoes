		...
		<table id="table-wrapper">
			...
			<c:forEach var="p" items="${products }" varStatus="i">
				<tr>
					<td>${i.getCount()}</td>
					<td>${p.getName()}</td>
					<td><img src="${p.getThumbnail().getThumbnail1()}"></td>
					<td><img src="${p.getThumbnail().getThumbnail2()}"></td>
					<td>${p.getColor()}</td>
					<td>${p.getPrice()}</td>
					...
					<td style="width: 200px;"><c:set var="id" value="${p.getId()}" />
						...
						<button type="button" class="btn btn-warning"
							...
							data-bs-target="#staticBackdrop${i.getCount()}">Update</button> <!-- Modal -->
										...
										<form:form action="WebShoes/product-manager/update1">
											...
											<div class="mb-3">
												<label>Thumbnail url 1:</label> <input type="text"
													name="thumbnail1" class="form-control" value="${p.getThumbnail().getThumbnail1()}">
											</div>
											<div class="mb-3">
												<label>Thumbnail url 2:</label> <input type="text"
													name="thumbnail2" class="form-control" value="${p.getThumbnail().getThumbnail2()}">
											</div>
								...
								<div class="modal-content">
									...
									<div class="modal-footer">
										...
										<button type="button" class="btn btn-danger"
											style="font-size: 23px;">Delete</button>
									</div>
								</div>
							</div>
						</div></td>
